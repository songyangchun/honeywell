...
public class ModifyPassWordActivity extends ToolBarActivity {
    ...
    public void onViewClicked() {
       ...
       List<LoginInfoBean> logininfolist= DataSupport.findAll(LoginInfoBean.class);
        boolean hasthisname = false;
        if(!logininfolist.isEmpty()) {
            for (int i = 0; i < logininfolist.size(); i++) {
                LoginInfoBean bean = logininfolist.get(i);
        ...
        DialogUtil.showAlertDialog(ModifyPassWordActivity.this, "提示", "修改密码成功.点击确认返回登录界面", new DialogInterface.OnClickListener() {
            @Override
